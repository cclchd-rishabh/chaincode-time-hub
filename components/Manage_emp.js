...
function ManageEmp() {
    ...
    // Fetch employees initially and after updates
    ...
    const handleClock = async (id, type) => {
        try {
            const response = await fetch(`/api/fetch?id=${id}`, {
                ...
                body: JSON.stringify({ [type]: true }), // Send either clock_in or clock_out
            });
    ...
    return (
        <div className="container mx-auto px-4">
            <h1 className="text-center text-2xl font-bold my-4">Manage Employees</h1>

            {employees.length > 0 ? (
                <div className="space-y-4">
                    {employees.map((emp) => (
                        <div key={emp.id} className="flex flex-col sm:flex-row items-center justify-between bg-white shadow-md p-4 rounded-lg border border-gray-200">
                            {/* Avatar and Details */}
                            ...
                            <div className="flex space-x-2">
                                ...
                                {!emp.clock_out && (
                                    <button
                                        ...
                                        disabled={!emp.clock_in || !!emp.clock}
                                    >
                                        Clock Out
                                    </button>
                                )}
